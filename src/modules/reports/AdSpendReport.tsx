          ...
          <table className="w-full text-xs">
            <thead>
              ...
              <tr className="bg-surface-50 dark:bg-dark-200/40 border-b border-surface-200 dark:border-dark-300">
                ...
                <th colSpan={4} className="px-2 py-1.5 text-center text-[11px] font-display font-bold text-blue-700 dark:text-blue-300 uppercase tracking-wide border-l border-surface-200 dark:border-dark-300">👔 Hombre</th>
                <th colSpan={4} className="px-2 py-1.5 text-center text-[11px] font-display font-bold text-rose-700 dark:text-rose-300 uppercase tracking-wide border-l border-surface-200 dark:border-dark-300">👗 Mujer</th>
                <th colSpan={4} className="px-2 py-1.5 text-center text-[11px] font-display font-bold text-emerald-700 dark:text-emerald-300 uppercase tracking-wide border-l border-surface-200 dark:border-dark-300">∑ Juntos</th>
              </tr>
              <tr className="bg-surface-50 dark:bg-dark-200/40 border-b border-surface-200 dark:border-dark-300 text-[10px] font-display font-semibold text-navy-500 dark:text-gray-400 uppercase">
                ...
                <th className="px-2 py-1.5 text-right border-l border-surface-200 dark:border-dark-300">Ventas</th>
                <th className="px-2 py-1.5 text-right">Pub.</th>
                ...
                rows.map((r) => {
                  ...
                  return (
                    <tr key={r.ymd} className={`border-b border-surface-100 dark:border-dark-300/40 ${isToday ? 'bg-amber-50/40 dark:bg-amber-900/10' : 'hover:bg-surface-50/60 dark:hover:bg-dark-200/30'}`}>
                      ...
                      {/* Hombre */}
                      <td className="px-2 py-1 text-right font-mono text-navy-800 dark:text-gray-200 border-l border-surface-100 dark:border-dark-300/40">{r.salesMen > 0 ? format(r.salesMen) : '—'}</td>
                      ...
                      <td className={`px-2 py-1 text-right font-mono ${r.netMen < 0 ? 'text-accent-red' : 'text-navy-800 dark:text-gray-200'}`}>{(r.spendMen > 0 || r.salesMen > 0) ? format(r.netMen) : '—'}</td>
                      <td className={`px-2 py-1 text-right font-mono ${r.pctMen !== null && r.pctMen > 50 ? 'text-accent-red font-bold' : 'text-navy-600 dark:text-gray-400'}`}>{r.pctMen !== null ? `${r.pctMen.toFixed(2)}%` : '—'}</td>

                      {/* Mujer */}
                      <td className="px-2 py-1 text-right font-mono text-navy-800 dark:text-gray-200 border-l border-surface-100 dark:border-dark-300/40">{r.salesWomen > 0 ? format(r.salesWomen) : '—'}</td>
                      <td className="px-1 py-0.5">
                        <input
                          type="number" min="0" step="0.01"
                          value={draft.women}
                      ...
                      <td className={`px-2 py-1 text-right font-mono ${r.netWomen < 0 ? 'text-accent-red' : 'text-navy-800 dark:text-gray-200'}`}>{(r.spendWomen > 0 || r.salesWomen > 0) ? format(r.netWomen) : '—'}</td>
                      <td className={`px-2 py-1 text-right font-mono ${r.pctWomen !== null && r.pctWomen > 50 ? 'text-accent-red font-bold' : 'text-navy-600 dark:text-gray-400'}`}>{r.pctWomen !== null ? `${r.pctWomen.toFixed(2)}%` : '—'}</td>

                      {/* Juntos (calculados) */}
                      <td className="px-2 py-1 text-right font-mono font-bold text-emerald-700 dark:text-emerald-300 border-l border-surface-100 dark:border-dark-300/40">{r.salesTotal > 0 ? format(r.salesTotal) : '—'}</td>
                      <td className="px-2 py-1 text-right font-mono font-semibold text-navy-700 dark:text-gray-300">{r.spendTotal > 0 ? format(r.spendTotal) : '—'}</td>
                      <td className={`px-2 py-1 text-right font-mono font-bold ${r.netTotal < 0 ? 'text-accent-red' : 'text-emerald-700 dark:text-emerald-300'}`}>{(r.spendTotal > 0 || r.salesTotal > 0) ? format(r.netTotal) : '—'}</td>
                      <td className={`px-2 py-1 text-right font-mono font-bold ${r.pctTotal !== null && r.pctTotal > 50 ? 'text-accent-red' : 'text-navy-700 dark:text-gray-300'}`}>{r.pctTotal !== null ? `${r.pctTotal.toFixed(2)}%` : '—'}</td>
                    </tr>
                  );
                })
              )}
            ...
            {!loading && (
              <tfoot>
                <tr className="bg-navy-50 dark:bg-dark-200/60 border-t-2 border-navy-300 dark:border-dark-400 text-xs font-display font-bold">
                  ...
                  <td className="px-2 py-2 text-right font-mono text-blue-700 dark:text-blue-300 border-l border-surface-200 dark:border-dark-300">{format(totals.salesMen)}</td>
                  <td className="px-2 py-2 text-right font-mono text-blue-700 dark:text-blue-300">{format(totals.spendMen)}</td>
                  <td className={`px-2 py-2 text-right font-mono ${totals.netMen < 0 ? 'text-accent-red' : 'text-blue-700 dark:text-blue-300'}`}>{format(totals.netMen)}</td>
                  <td className="px-2 py-2 text-right font-mono text-blue-700 dark:text-blue-300">{totals.pctMen !== null ? `${totals.pctMen.toFixed(2)}%` : '—'}</td>
                  <td className="px-2 py-2 text-right font-mono text-rose-700 dark:text-rose-300 border-l border-surface-200 dark:border-dark-300">{format(totals.salesWomen)}</td>
                  <td className="px-2 py-2 text-right font-mono text-rose-700 dark:text-rose-300">{format(totals.spendWomen)}</td>
                  <td className={`px-2 py-2 text-right font-mono ${totals.netWomen < 0 ? 'text-accent-red' : 'text-rose-700 dark:text-rose-300'}`}>{format(totals.netWomen)}</td>
                  <td className="px-2 py-2 text-right font-mono text-rose-700 dark:text-rose-300">{totals.pctWomen !== null ? `${totals.pctWomen.toFixed(2)}%` : '—'}</td>
                  <td className="px-2 py-2 text-right font-mono text-emerald-700 dark:text-emerald-300 border-l border-surface-200 dark:border-dark-300">{format(totals.salesTotal)}</td>
                  <td className="px-2 py-2 text-right font-mono text-emerald-700 dark:text-emerald-300">{format(totals.spendTotal)}</td>
                  <td className={`px-2 py-2 text-right font-mono ${totals.netTotal < 0 ? 'text-accent-red' : 'text-emerald-700 dark:text-emerald-300'}`}>{format(totals.netTotal)}</td>
                  <td className="px-2 py-2 text-right font-mono text-emerald-700 dark:text-emerald-300">{totals.pctTotal !== null ? `${totals.pctTotal.toFixed(2)}%` : '—'}</td>
                </tr>
              </tfoot>
            )}
          </table>
