...
interface DraftItem {
  ...
  color: string;
  variantIndex: number;
  /** Stock disponible en almacén al momento de armar la draft (referencial). */
...
export function CreateTransferModal({
  ...
  function handleAddItem(product: Product, variantIndex: number) {
    ...
    if (available <= 0) {
      toast.warning(`${product.name} ${variant.size}/${variant.color} no tiene stock en el almacén.`);
      return;
    }
    ...
    if (existing) {
      ...
      if (proposed > available) {
        toast.warning(`No podés mover más de ${available} unidades (todo el stock del almacén).`);
        return;
      }
    ...
    setItems((prev) => [
      ...prev,
      {
        ...
        color: variant.color,
        variantIndex,
        availableInWarehouse: available,
        quantitySent: 1,
      },
    ]);
  }
  ...
  async function handleSubmit() {
    ...
    const overflow = items.find((i) => i.quantitySent > i.availableInWarehouse);
    if (overflow) {
      toast.warning(
        `${overflow.productName} ${overflow.size}/${overflow.color}: cantidad excede el stock del almacén (${overflow.availableInWarehouse}).`,
      );
      return;
    }
    ...
    <Modal open onClose={onClose} title="Nueva transferencia" size="lg">
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2 px-3 py-2 bg-surface-50 dark:bg-dark-200/40 rounded-lg">
          ...
          <span className="text-xs font-display font-semibold text-navy-700 dark:text-gray-300">Almacén</span>
          <ArrowRight size={14} className="mx-1 text-navy-300" />
          <Store size={14} className="text-emerald-500" />
          <span className="text-xs font-display font-semibold text-navy-700 dark:text-gray-300">Tienda</span>
        ...
        <div>
          ...
          {items.length === 0 ? (
            <div className="border-2 border-dashed border-surface-200 dark:border-dark-300 rounded-lg p-6 text-center">
              ...
              <p className="text-xs text-navy-400 dark:text-gray-500">Agregá productos del almacén para transferir a la tienda.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-xs">
                <thead className="bg-surface-50 dark:bg-dark-200/40">
                  <tr>
                    ...
                    <th className="text-right px-3 py-2 font-display font-semibold text-navy-500 w-20">Disp.</th>
                ...
                <tbody className="divide-y divide-surface-100 dark:divide-dark-300/40">
                  {items.map((item, idx) => {
                    const tooMuch = item.quantitySent > item.availableInWarehouse;
                    return (
                      <tr key={idx}>
                        ...
                        <td className="px-3 py-2 text-right font-mono text-navy-500">{item.availableInWarehouse}</td>
                        <td className="px-3 py-2">
                          <input
                            type="number"
                            min="0"
                            max={item.availableInWarehouse}
...
function ProductPickerModal({
  ...
  const productsWithWarehouseStock = useMemo(() => {
    return products.filter((p) =>
      p.variants?.some((v) => (v.stockWarehouse ?? 0) > 0),
    );
  }, [products]);
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-navy-400">
              {productsWithWarehouseStock.length === 0
                ? 'No hay productos con stock en el almacén.'
                    ...
                    {variantsInStock.map((v) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          title={`Click para agregar 1 unidad (stock: ${v.stockWarehouse})`}
                        >
                          ...
                          <span className="ml-2 font-mono text-blue-600">×{v.stockWarehouse}</span>
                        </button>
                      );
                    })}
                  </div>
